...
# データの読み込み
imagefiles = np.load("imagefiles_augmented.npz")
...
model.save('leukemia_cnn_augmented.h5')
...
fig.savefig('result/cnn_augmented/cnn_augmented_accuracy.png')
plt.close()
# ロスの推移をプロット
fig = plt.figure()
...
plt.title('Loss')
plt.legend(['train', 'test'], loc='upper left')
fig.savefig('result/cnn_augmented/cnn_augmented_loss.png')
plt.close()
# Confusion matrix作成
plt.figure()
...
plt.tight_layout()
plt.savefig('result/cnn_augmented/confusion_matrix_cnn_augmented.png')
plt.close()
